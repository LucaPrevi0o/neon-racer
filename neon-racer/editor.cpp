...
void TrackEditor::CycleSelection() {
    ...
    if (found == pieces.end() || ++found == pieces.end()) selectedPieceId_ = pieces.front().id;
    else selectedPieceId_ = found->id;
...
void TrackEditor::PlacePreview() {
    ...
    SaveUndoState();
    const std::uint32_t id = track_.Add(candidate);
    if (id == 0) { undoStates_.pop_back(); SetMessage("Placement blocked: invalid component parameters."); return; }
    selectedPieceId_ = 0;
    preview_.id = 0;
...
void TrackEditor::TransformSelected() {
    ...
    TrackPiece replacement = BuildPreview();
    replacement.id = selectedPieceId_;
...
void TrackEditor::DeleteSelected() {
    if (selectedPieceId_ == 0) { SetMessage("Select a component first."); return; }
    SaveUndoState();
    if (!track_.RemovePiece(selectedPieceId_)) { undoStates_.pop_back(); return; }
    selectedPieceId_ = 0;
...
void TrackEditor::SetStartFinish() {
    if (!track_.IsLayoutValid()) {
        SetMessage("Finish placement unlocks after all connectors form one closed loop.");
        return;
    }
    const TrackPiece* selected = track_.GetPiece(selectedPieceId_);
    if (selected == 0 || selected->type != TrackPieceType::Straight) {
        SetMessage("Start/finish must be placed on a selected straight.");
        return;
    }
    SaveUndoState();
    track_.SetStartFinish(selectedPieceId_, RaceDirection::Forward);
...
void TrackEditor::DrawTrackLibrary() const {
    ...
    if (namingDraft_) {
        ...
        DrawText("Names save only into neon-racer/tracks/custom/.", 840, 265, 14, Fade(RAYWHITE, 0.68f));
        return;
    }
