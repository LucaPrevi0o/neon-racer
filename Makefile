...
ROUTE_PROGRESS_TEST := $(BUILD_DIR)/tests/route_progress_tests
...
ROUTE_PROGRESS_SOURCES := src/race/time_trial/route_progress.cpp

TIME_TRIAL_SOURCES := src/race/time_trial.cpp \
	$(ROUTE_PROGRESS_SOURCES) \
	$(RACE_GHOST_SOURCES) \
...
ROUTE_PROGRESS_TEST_SOURCES := tests/unit/test_route_progress.cpp \
	$(ROUTE_PROGRESS_SOURCES) \
	$(RACE_TRACK_SOURCES)
...
$(ROUTE_PROGRESS_TEST): $(ROUTE_PROGRESS_TEST_SOURCES)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(PROJECT_CXXFLAGS) -o $@ $(ROUTE_PROGRESS_TEST_SOURCES)
...
test racer-test: $(TRACK_TEST) $(RACE_PHYSICS_TEST) $(VEHICLE_DYNAMICS_TEST) $(GHOST_REPLAY_TEST) $(TIME_TRIAL_TEST) $(ROUTE_PROGRESS_TEST) $(PLAYABLE_IO_TEST) $(MAIN_MENU_FLOW_TEST) $(PIECE_CATALOG_TEST) $(REPOSITORY_HARDENING_TEST)
	./$(TRACK_TEST)
	./$(RACE_PHYSICS_TEST)
