...
PERSISTENCE_WORKFLOW_TEST := $(BUILD_DIR)/tests/persistence_workflow_tests
...
TRACK_POSITION_SOURCES := src/race/time_trial/track_position_geometry.cpp \
...
CHECKPOINT_RECOVERY_TEST_SOURCES := tests/unit/test_checkpoint_recovery.cpp \
	$(TIME_TRIAL_SOURCES) \
	$(RACE_TRACK_SOURCES)
...
PERSISTENCE_WORKFLOW_TEST_SOURCES := tests/unit/test_persistence_workflows.cpp \
	$(PLAYABLE_SOURCES) \
	$(PLAYABLE_PERSISTENCE_SOURCES) \
	$(DRAFT_PERSISTENCE_SOURCES) \
	$(RACE_GHOST_SOURCES) \
...
EDITOR_CONNECTOR_GUIDE_TEST_SOURCES := tests/unit/test_editor_connector_guides.cpp \
	$(RACE_TRACK_SOURCES)
...
$(CHECKPOINT_RECOVERY_TEST): $(CHECKPOINT_RECOVERY_TEST_SOURCES)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(PROJECT_CXXFLAGS) -o $@ $(CHECKPOINT_RECOVERY_TEST_SOURCES)

$(PLAYABLE_IO_TEST): $(PLAYABLE_IO_TEST_SOURCES)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(PROJECT_CXXFLAGS) -o $@ $(PLAYABLE_IO_TEST_SOURCES)

$(PERSISTENCE_WORKFLOW_TEST): $(PERSISTENCE_WORKFLOW_TEST_SOURCES)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(PROJECT_CXXFLAGS) -o $@ $(PERSISTENCE_WORKFLOW_TEST_SOURCES)
...
$(EDITOR_CONNECTOR_GUIDE_TEST): $(EDITOR_CONNECTOR_GUIDE_TEST_SOURCES)
	@mkdir -p $(dir $@)
	$(CXX) $(CXXFLAGS) $(PROJECT_CXXFLAGS) -o $@ $(EDITOR_CONNECTOR_GUIDE_TEST_SOURCES)
...
test racer-test: $(TRACK_TEST) $(TRACK_PROGRESS_GRAPH_TEST) $(RACE_PHYSICS_TEST) $(VEHICLE_DYNAMICS_TEST) $(GHOST_REPLAY_TEST) $(TIME_TRIAL_TEST) $(TRACK_POSITION_TEST) $(CHECKPOINT_RECOVERY_TEST) $(PLAYABLE_IO_TEST) $(PERSISTENCE_WORKFLOW_TEST) $(MAIN_MENU_FLOW_TEST) $(PLAYABLE_LIBRARY_FLOW_TEST) $(RACE_PAUSE_FLOW_TEST) $(RACE_RESULTS_FLOW_TEST) $(PIECE_CATALOG_TEST) $(EDITOR_CONNECTOR_GUIDE_TEST) $(REPOSITORY_HARDENING_TEST)
	./$(TRACK_TEST)
